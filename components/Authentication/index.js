        ...
        <div className="w-full min-h-screen flex flex-col md:flex-row">
            ...
            <div className="authentication w-full md:w-7/12 p-8 flex justify-center items-center relative select-none">
                <div className="hidden md:block">
                    <VictoryCard victoryData={victories[0]} color={'blue'} offset={'top-1/4 left-4'} animationDur={'4s'} />
                    <VictoryCard victoryData={victories[1]} color={'yellow'} offset={'top-4 right-1/4'} animationDur={'3s'} />
                    <VictoryCard victoryData={victories[2]} color={'red'} offset={'top-3/4 left-2/4'} animationDur={'7s'} />
                    <VictoryCard victoryData={victories[3]} color={'green'} offset={'bottom-16 left-12'} animationDur={'5s'} />
                </div>
...
const VictoryCard = ({victoryData, color, offset, animationDur}) => {
    ...
    return (
        <div className={`w-64 h-max absolute ${offset} rounded-xl shadow animate-lighthouse bg-gray-100`} style={{'animationDuration': animationDur}}>
            <div className="w-full h-full relative flex flex-col justify-between rounded-xl bg-gray-100">
                <div>
                    <h1 className={`p-2 text-xl rounded-xl border-b border-gray-400 bg-${color}-200`}>{victoryData && victoryData.title}</h1>
                    <p className="pl-4 py-2">{victoryData && parseDescr(victoryData.description)}</p>
                </div>
                <div className="flex justify-between items-center p-2">
                    <p>@{victoryData && victoryData.author}</p>
                    <p>{Date(victoryData && victoryData.date_created).slice(0,16)}</p>
                </div>
            </div>
        </div>
    )
